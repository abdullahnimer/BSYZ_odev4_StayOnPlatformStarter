...
public class PopulationManager_sc : MonoBehaviour
{
    ...
    GameObject Breed(GameObject prant1,GameObject prant2){
        ...
        if(Random.Range(0,100)==1)// mutate 1 in 100
        {
            ...
            b.dna_sc.Combine(prant1.GetComponent<Brain_sc>().dna_sc,
                                prant2.GetComponent<Brain_sc>().dna_sc);
        }
        return offspring;
    }

    void BreedNewPopulation (){// distanceTravelled hesaplama
        List<GameObject> sortedList = 
        population.OrderBy(x => x.GetComponent<Brain_sc>().timeAlive).ToList();
        population.Clear();
